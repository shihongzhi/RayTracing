...
int Matrix::Inverse(Matrix &m, float epsilon) const {
  ...
  for (i = 0; i < 4; i++) {
	...
	return 0;
      ...
      MultRow(j,row_factor,tmp,m);
      SubtractRow(j,i,tmp,m);
    }
  }
  for (i = 0; i < 4; i++) {
    ...
    MultRow(i,row_factor,tmp,m);
  }
  return 1;
}

// HELPERS
void Matrix::MultRow(int row, float row_factor, Matrix &a, Matrix &b) {
  for (int i = 0; i < 4; i++) {
    a.data[row][i] *= row_factor;
    b.data[row][i] *= row_factor;
  }
}

void Matrix::SubtractRow(int row1, int row2, Matrix &a, Matrix &b) {
  for (int i = 0; i < 4; i++) {
    a.data[row1][i] -= a.data[row2][i];
    b.data[row1][i] -= b.data[row2][i];
  }
}
